...
class RemoveConcats(val options: TranslationOptions) extends DescriptionBasedTransform {
  ...
  def processDescription(d: Description): Description = {
    ...
    def getStatement(a: ArrayBuffer[Statement], s: Statement): Statement = {
      (a.isEmpty, s) match {
        case (true, _) => s
        case _ => SimpleBlock(ui, a.toSeq ++ Seq(s))
      }
    }
    ...
    def processConnect(c: Connect): Statement = {
      ...
      if (preStmts.isEmpty && postStmts.isEmpty) {
        ...
        SimpleBlock(connect.tokens, pre.toSeq ++ Seq(connect) ++ post.toSeq)
      }
    }
    ...
    def processDefLogic(d: DefLogic): Statement = {
      ...
      if (preStmts.isEmpty) {
        ...
        // clean any token in the statement to avoid issues with comment re-integration
        val pre = preStmts.map(Utils.cleanTokens).toSeq
    ...
    def processInstance(i: DefInstance): Statement = {
      ...
      if (preStmts.isEmpty && postStmts.isEmpty) {
        ...
        // clean any token in the statement to avoid issues with comment re-integration
        val pre = preStmts.map(Utils.cleanTokens).toSeq
        val post = postStmts.map(Utils.cleanTokens).toSeq
