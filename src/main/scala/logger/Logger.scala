// Freely inspired by firrtl Logger.scala
...
object LogLevel extends Enumeration {
  val Error, Warn, Struct, Info, Debug, Trace, None = Value

  def apply(s: String): LogLevel.Value = s.toLowerCase match {
    case "error" => LogLevel.Error
    case "warn"  => LogLevel.Warn
    case "struct"  => LogLevel.Struct
...
class Logger(containerClass: String) {
  ...
  /**
    ...
    * @param message message generator to be invoked if level is right
    */
  def struct(message: => String): Unit = {
    Logger.showMessage(LogLevel.Struct, containerClass, message)
  }
  /**
